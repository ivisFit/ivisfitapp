...
import {
  SaveSiteContentSchema,
  SiteContentLocaleSchema,
} from "@/lib/preview-cms/types/site-content";
import {
...
export async function PUT(req: NextRequest, ctx: RouteCtx) {
  try {
    ...
    if (!localeResult.success) {
      return NextResponse.json(
        { message: "Locale inválido", code: "BAD_REQUEST" },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { message: "JSON inválido", code: "BAD_REQUEST" },
        { status: 400 },
      );
    }
    ...
    const result = await saveSiteContentLocale(
      localeResult.data,
      session.user.id,
      parsed.data,
    );
    if (!result.ok) {
      return NextResponse.json(
        { message: result.error, code: "BAD_REQUEST" },
        { status: 400 },
      );
    }
    return NextResponse.json({

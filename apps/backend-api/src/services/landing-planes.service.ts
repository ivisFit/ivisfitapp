...
const defaultLandingPlans: CreateLandingPlanInput[] = [
  {
    slug: "gluteos",
    orden: 1,
...
async function ensureDefaultLandingPlans() {
  await Promise.all(
    defaultLandingPlans.map((plan) =>
      LandingPlan.updateOne(
        { slug: plan.slug },
        { $setOnInsert: plan },
        { upsert: true },
      ),
...
export const landingPlanesService = {
  async listPublic() {
    await ensureDefaultLandingPlans();
    return LandingPlan.find({ isActive: true }).sort({ orden: 1 });
  },

  async listAll() {
    await ensureDefaultLandingPlans();
    return LandingPlan.find().sort({ orden: 1 });
  },

  async getByIdOrSlug(idOrSlug: string) {
    await ensureDefaultLandingPlans();

    const plan = isObjectId(idOrSlug)
      ? await LandingPlan.findById(idOrSlug)
      : await LandingPlan.findOne({ slug: idOrSlug });

    return assertFound(plan, "Plan no encontrado");
  },

  async create(data: CreateLandingPlanInput) {
    try {
      return await LandingPlan.create(data);
    } catch (error) {
      if (isDuplicateKeyError(error)) {
        throw new AppError(409, "Ya existe un plan con ese identificador o ruta");
      }
      throw error;
    }
  },

  async update(id: string, data: UpdateLandingPlanInput) {
    try {
      const plan = await LandingPlan.findByIdAndUpdate(id, data, {
        new: true,
        runValidators: true,
      });
      return assertFound(plan, "Plan no encontrado");
  ...
  async remove(id: string) {
    const plan = await LandingPlan.findByIdAndDelete(id);
    return assertFound(plan, "Plan no encontrado");
  },
};
